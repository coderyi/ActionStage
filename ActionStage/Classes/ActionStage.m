...
static const char *graphQueueSpecific = "com.actionstage.graphdispatchqueue";
...
@interface ASWatcherPathPair : NSObject
@property (nonatomic, strong) ASHandle *watcher;
@property (nonatomic, copy) NSString *path;
@end

@implementation ASWatcherPathPair
@end

@interface ActionStage ()
{
    NSMutableArray *_removeWatcherFromPathRequests;
    NSMutableArray *_removeWatcherRequests;
}
...
- (id)init
{
    ...
    if (self != nil)
    {
        ...
        _removeWatcherRequests = [NSMutableArray array];
        _removeWatcherFromPathRequests = [NSMutableArray array];
    }
    return self;
}
...
- (dispatch_queue_t)globalStageDispatchQueue
{
    ...
    {
        mainGraphQueue = dispatch_queue_create("com.actionstage.graphdispatchqueue", 0);
        
        globalGraphQueue = dispatch_queue_create("com.actionstage.graphdispatchqueue-global", 0);
        dispatch_set_target_queue(globalGraphQueue, mainGraphQueue);
        
        highPriorityGraphQueue = dispatch_queue_create("com.actionstage.graphdispatchqueue-high", 0);
...
#endif
{
    ...
    if (isGraphQueue)
    {
        ...
        if (executionTime > 0.1)
            ASLog(@"***** Dispatch from %s:%d took %f s", function, line, executionTime);
#endif
    }
    else
    {
...
- (void)dumpGraphState
{
    [self dispatchOnStageQueue:^
     {
         ASLog(@"===== SGraph State =====");
         ASLog(@"%d live node watchers", _liveNodeWatchers.count);
         [_liveNodeWatchers enumerateKeysAndObjectsUsingBlock:^(NSString *path, NSArray *watchers, __unused BOOL *stop)
          {
              ...
              for (ASHandle *handle in watchers)
              {
                  ...
                  if (watcher != nil)
                  {
                      ASLog(@"        %@", [watcher description]);
                  }
              }
          }];
         ASLog(@"%d requests", _activeRequests.count);
...
- (NSString *)optionsHash:(NSDictionary *)options
{
    ...
    for (NSString *key in keys)
    {
        ...
        {
            ASLog(@"Warning: optionsHash: key is not a string");
            continue;
        }
...
- (void)_requestGeneric:(bool)joinOnly inCurrentQueue:(bool)inCurrentQueue path:(NSString *)path options:(NSDictionary *)options flags:(int)flags watcher:(id<ASWatcher>)watcher
{
    ...
    dispatch_block_t requestBlock = ^
    {
        ...
        {
            ASLog(@"Error: %s:%d: actionHandle.delegate is nil", __PRETTY_FUNCTION__, __LINE__);
            return;
        }
        ...
        if (cancelRequestInfo != nil) // 启动该path前把取消信息删除
        {
            ASTimer *timer = [cancelRequestInfo objectForKey:@"timer"];
            [timer invalidate];
            timer = nil;
            ...
            ASLog(@"Resuming request to \"%@\"", path);
        }
        ...
        if (requestInfo == nil)
        {
            ...
            if (requestBuilder != nil)
            {
                ...
                if (requestBuilder.requestQueueName != nil)
                {
                    ...
                    else
                    {
                        ...
                        {
                            executeNow = false;
                            ASLog(@"Adding request %@ to request queue \"%@\"", requestBuilder, requestBuilder.requestQueueName);
                            
                            if (flags & ASActorRequestChangePriority)
                            {
                                if (requestQueue.count > 2)
                                {
                                    ...
                                    ASLog(@"(Inserted actor with high priority (next in queue)");
                                }
                            }
                        }
                    }
            ...
            {
                ASLog(@"Error: request builder not found for \"%@\"", path);
            }
        }
        else
        {
            ...
            {
                ASLog(@"Joining watcher to the watchers of \"%@\"", path);
                [watchers addObject:actionHandle];
            }
            else
            {
                ASLog(@"Continue to watch for actor \"%@\"", path);
            }
             ...
             if (actor.requestQueueName != nil)
             {
                 ...
                 if (requestQueue != nil && requestQueue.count != 0)
                 {
                     ...
                     if (index != NSNotFound && index != 0 && index != 1)
                     {
                         ...
                         ASLog(@"Changed actor %@ priority (next in %@)", path, actor.requestQueueName);
                     }
                 }
             }
         }
...
- (bool)isExecutingActorsWithGenericPath:(NSString *)genericPath
{
    if (![self isCurrentQueueStageQueue])
    {
        ASLog(@"%s should be called from graph queue", __PRETTY_FUNCTION__);
...
- (void)watchForPath:(NSString *)path watcher:(id<ASWatcher>)watcher
{
    ...
    if (actionHandle == nil)
    {
        ASLog(@"***** Warning: actionHandle is nil in %s:%d", __PRETTY_FUNCTION__, __LINE__);
        return;
    }
...
- (void)removeRequestFromQueueAndProceedIfFirst:(NSString *)name fromRequestBuilder:(ASActor *)requestBuilder
{
    ...
    if (requestQueue == nil)
        ASLog(@"Warning: requestQueue is nil");
    else
    {
        if (requestQueue.count == 0)
        {
            ASLog(@"***** Warning ***** request queue \"%@\" is empty.", requestBuilder.requestQueueName);
        }
        else
        {
            ...
            else
            {
                ...
                {
                    ASLog(@"***** Warning ***** request queue \"%@\" doesn't contain request to %@", requestBuilder.requestQueueName, requestBuilder.path);
                }
            }
        }
    }
...
- (void)removeWatcherByHandle:(ASHandle *)actionHandle
{
    ...
    {
        ASLog(@"***** Warning: graph handle is nil in removeWatcher");
        return;
    }
    
    bool alreadyExecuting = false;
    OSSpinLockLock(&removeWatcherRequestsLock);
    if (_removeWatcherRequests.count > 0)
        alreadyExecuting = true;
    [_removeWatcherRequests addObject:watcherGraphHandle];
    ...
    [self dispatchOnHighPriorityQueue:^
     {
         NSMutableArray *removeWatchers = [NSMutableArray array];
         
         OSSpinLockLock(&removeWatcherRequestsLock);
         [removeWatchers addObjectsFromArray:_removeWatcherRequests];
         [_removeWatcherRequests removeAllObjects];
         OSSpinLockUnlock(&removeWatcherRequestsLock);
         for (NSInteger i = 0; i < removeWatchers.count; i++)
         {
             ASHandle *actionHandle = [removeWatchers objectAtIndex:i];
...
- (void)removeWatcherByHandle:(ASHandle *)watcherGraphHandle fromPath:(NSString *)watcherPath
{
    if (watcherGraphHandle == nil)
    {
        ASLog(@"***** Warning: graph handle is nil in removeWatcher:fromPath");
        return;
    }
    
    bool alreadyExecuting = false;
    OSSpinLockLock(&removeWatcherFromPathRequestsLock);
    if (_removeWatcherFromPathRequests.count > 0)
        alreadyExecuting = true;
    ASWatcherPathPair *pair = [[ASWatcherPathPair alloc] init];
    pair.watcher = watcherGraphHandle;
    pair.path = watcherPath;
    [_removeWatcherFromPathRequests addObject:pair];
    ...
    [self dispatchOnHighPriorityQueue:^
     {
         NSMutableArray *removeWatchersFromPath = [NSMutableArray array];
         
         OSSpinLockLock(&removeWatcherFromPathRequestsLock);
         [removeWatchersFromPath addObjectsFromArray:_removeWatcherFromPathRequests];
         [_removeWatcherFromPathRequests removeAllObjects];
         OSSpinLockUnlock(&removeWatcherFromPathRequestsLock);
         
         if (removeWatchersFromPath.count > 1)
         {
             ASLog(@"Cancelled %ld requests at once", removeWatchersFromPath.count);
         }
         for (NSInteger i = 0; i < removeWatchersFromPath.count; i++)
         {
             ASWatcherPathPair *watcherPathPair = [removeWatchersFromPath objectAtIndex:i];
             ASHandle *actionHandle = watcherPathPair.watcher;
             NSString *path = watcherPathPair.path;
             if (path == nil)
                 continue;
    ...
    [self dispatchOnStageQueue:^
     {
         ...
         if (requestInfo != nil)
         {
             ...
             if (requestBuilder == nil)
                 ASLog(@"***** Warning ***** requestBuilder is nil");
...
- (void)scheduleCancelRequest:(NSString *)path
{
    ...
    if (requestInfo != nil && cancelRequestInfo == nil)
    {
        ...
        if (cancelTimeout <= DBL_EPSILON)
        {
            ...
            ASLog(@"Cancelled request to \"%@\"", path);
            if (requestBuilder.requestQueueName != nil)
                [self removeRequestFromQueueAndProceedIfFirst:requestBuilder.requestQueueName fromRequestBuilder:requestBuilder];
        }
        else
        {
            ASLog(@"Will cancel request to \"%@\" in %f s", path, cancelTimeout);
            NSDictionary *cancelDict = [NSDictionary dictionaryWithObjectsAndKeys:path, @"path", [NSNumber numberWithInt:0], @"type", nil];
            ASTimer *timer = [[ASTimer alloc] initWithTimeout:cancelTimeout repeat:false completion:^
                             {
    ...
    {
        ASLog(@"Warning: cannot cancel request to \"%@\": no active request found", path);
    }
}
    ...
    [self dispatchOnStageQueue:^
     {
         ...
         if (cancelRequestInfo == nil)
         {
             ASLog(@"Warning: cancelNodeRequestTimerEvent: \"%@\": no cancel info found", path);
             return;
         }
         ...
         if (requestBuilder == nil)
         {
             ASLog(@"Warning: active request builder for \"%@\" not fond, cannot cancel request", path);
         }
         else
         {
             [requestBuilder cancel];
